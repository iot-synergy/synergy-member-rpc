...
func (cq *CommentQuery) Clone() *CommentQuery {
	...
	return &CommentQuery{
		...
		withReplys: cq.withReplys.Clone(),
...
// WithReplys tells the query-builder to eager-load the nodes that are connected to
// the "replys" edge. The optional arguments are used to configure the query builder of the edge.
...
func (cq *CommentQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Comment, error) {
	...
	if query := cq.withReplys; query != nil {
		if err := cq.loadReplys(ctx, query, nodes,
			func(n *Comment) { n.Edges.Replys = []*Reply{} },
			func(n *Comment, e *Reply) { n.Edges.Replys = append(n.Edges.Replys, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (cq *CommentQuery) loadReplys(ctx context.Context, query *ReplyQuery, nodes []*Comment, init func(*Comment), assign func(*Comment, *Reply)) error {
	...
	query.Where(predicate.Reply(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(comment.ReplysColumn), fks...))
	}))
