...
func (cc *CommentCreate) defaults() {
	...
	if _, ok := cc.mutation.IsReply(); !ok {
		v := comment.DefaultIsReply
		cc.mutation.SetIsReply(v)
	}
}
...
func (cc *CommentCreate) check() error {
	...
	if _, ok := cc.mutation.MemberId(); !ok {
		return &ValidationError{Name: "memberId", err: errors.New(`ent: missing required field "Comment.memberId"`)}
	}
	if v, ok := cc.mutation.MemberId(); ok {
	...
	if _, ok := cc.mutation.IsReply(); !ok {
		return &ValidationError{Name: "is_reply", err: errors.New(`ent: missing required field "Comment.is_reply"`)}
	}
	return nil
}
...
func (cc *CommentCreate) createSpec() (*Comment, *sqlgraph.CreateSpec) {
	...
	if value, ok := cc.mutation.MemberId(); ok {
	...
	if value, ok := cc.mutation.IsReply(); ok {
		_spec.SetField(comment.FieldIsReply, field.TypeBool, value)
		_node.IsReply = value
	}
	if nodes := cc.mutation.ReplysIDs(); len(nodes) > 0 {
